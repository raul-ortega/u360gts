...
static uint8_t CHAR_FORMAT = NORMAL_CHAR_FORMAT;
...
// Init low-level driver and clear display buffer
// All operations like printing text or RLE bitmap, setting cursor position, etc
// are performed with buffer.
// To display contents of buffer OLED_flush() must be called.
// Note - flush operation may take some time, breaking realtime operation.
bool OLED_init(uint8_t oled_type)
{
    // Clear buffer
    OLED_clear_display();
    // Init low-level driver
    return ug2864hsweg01_Init(oled_type);
}

// Output buffer to physical display
void OLED_flush(void)
{
    ug2864hsweg01_Display((const uint8_t *)buffer);
}
...
// Set position for text output, arbitrary
// 0 <= column <= SCREEN_CHARACTER_COLUMN_COUNT - 1
// 0 <= row <= SCREEN_CHARACTER_ROW_COUNT - 1
void OLED_set_xy(uint8_t column, uint8_t row)
{
    curr_row = row;
    if (curr_row > (SCREEN_CHARACTER_ROW_COUNT - 1))
    {
    	// Error. Prevent from corrupting the memory
    	curr_row = 0;
    }
    curr_col = column * CHARACTER_WIDTH_TOTAL;
    if (curr_col > (OLED_WIDTH - 1))
    {
    	// Error. Prevent from corrupting the memory
    	curr_col = 0;
    }
...
// Set position for text output, from the beginning of the line
void OLED_set_line(uint8_t row)
{
	OLED_set_xy(0, row);
}

static void i2c_OLED_write_byte(uint8_t byte)
{
    buffer[curr_row * OLED_WIDTH + curr_col] = byte;
    curr_col++;
    if (curr_col > (OLED_WIDTH - 1))
	{
		// Wrap column index
...
void OLED_send_charH(unsigned char ascii, bool highlight)
{
    unsigned char i;
    uint8_t tmp;
    for (i = 0; i < 5; i++)
    {
        if (highlight)
            tmp = ~multiWiiFont[ascii - 32][i];
        else
            tmp = multiWiiFont[ascii - 32][i];
        tmp ^= CHAR_FORMAT; 
        i2c_OLED_write_byte(tmp);
    }
...
{
    OLED_send_charH(ascii, false);
}

void OLED_send_stringH(const char *string, bool highlight)
{
    // Sends a string of chars until null terminator
    while (*string)
    {
        OLED_send_charH(*string, highlight);
        string++;
    }
}
...
{
    OLED_send_stringH(string, false);
}
...
void OLED_Render_RLE_Bitmap(uint8_t *bitmap)
{
    ...
    {
        OLED_set_line(2 + i); // Set hi col addr

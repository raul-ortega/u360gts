...
void resetSerialConfig(serialConfig_t *serialConfig)
{
    ...
    /*//Serial 1
    serialConfig->portConfigs[0].identifier = SERIAL_PORT_USART1;
    serialConfig->portConfigs[0].msp_baudrateIndex = BAUD_9600;
    serialConfig->portConfigs[0].functionMask = FUNCTION_NONE;
    //Serial 2
    serialConfig->portConfigs[1].identifier = SERIAL_PORT_USART2;
    serialConfig->portConfigs[1].gps_baudrateIndex = BAUD_9600;
    serialConfig->portConfigs[1].functionMask = FUNCTION_GPS;
    //Softserial 1
    serialConfig->portConfigs[2].identifier = SERIAL_PORT_SOFTSERIAL1;
    serialConfig->portConfigs[2].telemetry_baudrateIndex = BAUD_9600;
    serialConfig->portConfigs[2].functionMask = FUNCTION_TELEMETRY_MFD;*/
...
/////////////////////////////
    ...
    masterConfig.serialConfig.portConfigs[2].functionMask = FUNCTION_TELEMETRY_NMEA;*/

    //masterConfig.serialConfig.portConfigs[0].

    //masterConfig.ena
    //PID
    masterConfig.p = 2500;//5000;
    masterConfig.i = 20;//100;
    masterConfig.d = 250;//1000;
    masterConfig.max_pid_error = 10;
    masterConfig.max_pid_accumulator = 5000;
    masterConfig.max_pid_gain = 500;
...
/*#if defined(COLIBRI_RACE)
    ...
    if(featureConfigured(FEATURE_EASING) && masterConfig.easing==0)
    	masterConfig.easing=1;
}

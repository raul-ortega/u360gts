...
/*#ifdef USE_SERIAL_1WIRE
	...
	CLI_COMMAND_DEF("dump", "dump configuration",NULL, cliDumpTracker),
...
const clivalue_t valueTable[] = {
	...
	{ "rssi_zoom",                  VAR_UINT8  | MASTER_VALUE,  &masterConfig.rxConfig.rssi_zoom, .config.minmax = { 1,  100 } },
...
*/
#ifdef GPS
	{ "gps_baud",               	VAR_UINT8  | MASTER_VALUE, &masterConfig.serialConfig.portConfigs[1].gps_baudrateIndex, .config.minmax = { BAUD_4800,  BAUD_250000 } },
...
*/
	...
	{ "max_pid_error",           	VAR_UINT8  | MASTER_VALUE, &masterConfig.max_pid_error, .config.minmax = { 0,  100 } },
	...
	{ "tilt_max_angle",         	VAR_UINT8  | MASTER_VALUE, &masterConfig.tilt_max_angle, .config.minmax = { 0,  90 } },
	{ "easing",           			VAR_UINT8  | MASTER_VALUE, &masterConfig.easing, .config.minmax = { 1,  4 } }, //0: no easing, 1: out-quart function, 2: out-circ function
    ...
    { "telemetry_inversion_in",     VAR_UINT8  | MASTER_VALUE | MODE_LOOKUP, &masterConfig.telemetry_inversion_in, .config.lookup = { TABLE_OFF_ON } },
    { "telemetry_inversion_out",    VAR_UINT8  | HIDDEN_VALUE | MODE_LOOKUP,  &masterConfig.telemetryConfig.telemetry_inversion_out, .config.lookup = { TABLE_OFF_ON } },
	...
	{ "eps_speed_gain",     		VAR_UINT16 | MASTER_VALUE, &masterConfig.eps_gain.speed, .config.minmax = { 1,  1000 } },
...
static void cliSerial(char *cmdline)
{
    ...
    for (i = 0; i < 4; i ++) {
        ...
        switch(i) {
            case 0:
                if (baudRateIndex < BAUD_1200 || baudRateIndex > BAUD_250000) {
                    continue;
                }
                portConfig.msp_baudrateIndex = baudRateIndex;
                break;
            case 1:
                if (baudRateIndex < BAUD_4800 || baudRateIndex > BAUD_115200) {
                    continue;
                }
                portConfig.gps_baudrateIndex = baudRateIndex;
                break;
            case 2:
                if (baudRateIndex != BAUD_AUTO && baudRateIndex > BAUD_250000) {
                    continue;
                }
...
#define DUMP_ALL (DUMP_MASTER | DUMP_PROFILE | DUMP_CONTROL_RATE_PROFILE | DUMP_TRACKER )
...
/*static void cliDump(char *cmdline)
{
    ...
    if (strcasecmp(cmdline, "tracker") == 0) {
        dumpMask = DUMP_TRACKER; // only
    }
...
void cliEnter(serialPort_t *serialPort)
{
    ...
    if(masterConfig.pan0_calibrated==0) cliPrint("\r\nYou must adjust the pan0 value\r\n");
}

static void cliExit(char *cmdline)
{
	DISABLE_PROTOCOL(TP_SERVOTEST);
	ENABLE_PROTOCOL(masterConfig.telemetry_protocol);
...
static void cliMovePanServo(char *cmdline)
{
	int16_t degrees;
    if (isEmpty(cmdline)) {
        cliPrint("Please specify the destination angle in degrees.\r\n");
        return;
    } else {
    	...
    	if(degrees>=0 && degrees<=360 ) {
    		...
    		printf("Angle %u is out of range.\r\n",degrees);
    	}
	}
}

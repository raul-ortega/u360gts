...
void i2c_OLED_send_char(unsigned char ascii){
	i2c_OLED_send_charH(ascii,false);
}

void i2c_OLED_send_stringH(const char *string, bool highlight)
{
    // Sends a string of chars until null terminator
    while (*string) {
        i2c_OLED_send_charH(*string,highlight);
        string++;
    }
}
void i2c_OLED_send_string(const char *string){
	i2c_OLED_send_stringH(string,false);
}
/**
